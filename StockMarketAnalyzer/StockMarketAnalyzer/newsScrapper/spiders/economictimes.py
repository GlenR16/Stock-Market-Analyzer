...
def get_urls():
    ...
    stock_ids_dict = {
        v: k.code
        for (k, v) in zip(
            stocks, [get_et_code(stock.code.split(".")[0]) for stock in stocks]
        )
    }
    urls = [
...
class NewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ecomonic_times_code = re.search(r"/companyid-(.*)\.cms", response.url)
        rel_stock = (
            stock_ids_dict[ecomonic_times_code.group(1)]
            if ecomonic_times_code
            else None
        )
        ...
        for story in stories:
            ...
            link = "https://economictimes.indiatimes.com" + str(
    ...
    def extract_article(self, response):
        ...
        news["date"] = datetime.strptime(
            response.meta.get("date").split(",")[0], "%d %b %Y"
        )
...
# convert above code to match the format of livemint.py spider ie use of response.css instead of Selector(text=response.body).xpath
...
#         stories = response.css("div")
#         if stories == None or stories == "":
#             yield {"Not connected"}
#         for story in stories:
#             title = story.css("h3 a::text").get()
#             link = "https://economictimes.indiatimes.com" + \
#                 str(story.css("h3 a::attr(href)").get())
#             type = str(story.css("span::text").get()).split("| ")[1]
#             date=story.css("div time::text").get()
#             summary=story.css("p::text").get()
#             yield scrapy.Request(
#                 link,
#                 callback=self.extract_article,
#                 meta={
...
#     def extract_article(self, response):
#         html=response.css("div.artText").get()
#         if html is None:
#             return
#         id = response.url.split("/")[-1].split(".")[0]
#         data=text_from_html(html)

...
CONT_DIV = '//*[@id="mc_mainWrapper"]/div[2]/div[2]/div[3]/div[2]/div[2]/div/div[3]/div[1]/div[not(@id="common_ge_widget_pricechart_news")]'
...
class NewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        rel_stock = stock_ids_dict[
            re.search(r"\?sc_id=(.*)\&durationType=", response.url).group(1)
        ]
        news_cont = Selector(text=response.body).xpath(CONT_DIV).getall()
        for news in news_cont:
            title = Selector(text=news).xpath(TITLE).get()
            link = "https://www.moneycontrol.com" + str(
                Selector(text=news).xpath(LINK).get()
            )
            if Selector(text=news).xpath(DATETIME).get() == None:
                continue
            temp = str(Selector(text=news).xpath(DATETIME).get()).strip(
                "\xa0|\xa0&nbsp;|&nbsp; Source: "
            )
    ...
    def extract_article(self, response):
        ...
        desc = (
            ...
            .xpath(f"//*[@id='article-{id}']/h2/text()")
            .get()
        )
        html = (
            scrapy.Selector(text=response.body)
            .xpath(f"//*[@id='article-{id}']//p/text()")
            .getall()
        )
        data = " ".join(html)
